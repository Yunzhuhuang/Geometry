...
void connectNeighbors(Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if (!circleListCheck.hit && distBetween <= maxLength){
        neighbors[i].add(j);
      }
    }
  }
...
//Convert a point to a Node in the graph
int posToNode(Vec2 point, Vec2[] centers, float[] radii, int numObstacles) { //To transfer a point to a node in the graph(if the point is already in the graph, just 
  for(int i = 0; i < numNodes; i++) {                                        //return it. Else, we add it to the graph and do the connectneighbors again
    if(nodePos[i].x == point.x && nodePos[i].y == point.y){
       return i;
    }
  }
...
ArrayList<Integer> planPath(Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  ...
  int startID = posToNode(startPos, centers, radii, numObstacles);
  if(addNodes == true){
    numNodes++;
    addNodes = false;
  }
  int goalID = posToNode(goalPos, centers, radii, numObstacles);
  if(addNodes == true){
    numNodes++;
    addNodes = false;
  }
  path = runASearch(nodePos, numNodes, startID, goalID);
  return path;
}

//Entry class for element put in the priority queue
...
//A* 
ArrayList<Integer> runASearch(Vec2[] nodePos, int numNodes, int startID, int goalID){
 ...
 for (int i = 0; i < numNodes; i++) { //Clear visit tags and parent pointers
  ...
  fringe.add(new Entry(startID, 0.0));
  //println("Adding node", startID, "(start) to the fringe.");
  //println(" Current Fringe: ", fringe);
  
  while (fringe.size() > 0){
    Entry currentNode = fringe.poll();
   
    if (currentNode.getKey() == goalID){
      //println("Goal found!");
      break;
    }
    for (int i = 0; i < neighbors[currentNode.getKey()].size(); i++){
      int neighborNode = neighbors[currentNode.getKey()].get(i);
      Float cost = nodePos[currentNode.getKey()].distanceTo(nodePos[neighborNode]) + nodePos[neighborNode].distanceTo(nodePos[goalID]);
      if (!visited[neighborNode]){
        visited[neighborNode] = true;
        parent[neighborNode] = currentNode.getKey();
        fringe.add(new Entry(neighborNode, cost));
